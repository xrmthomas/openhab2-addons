...
public class Ipx800Handler extends BaseBridgeHandler implements Ipx800EventListener {
    ...
    private class LongPressEvaluator implements Runnable {
        ...
        public LongPressEvaluator(ChannelUID uid, String port, PortData portData, boolean swap) {
        ...
        public void run() {
            ...
            if (((currentData.value == 1) || (currentData.value == 0 && swap))
                    && currentData.timestamp == referenceTime) {
                triggerChannel(uid + "#" + CHANNEL_TYPE_PUSH_BUTTON_TRIGGER, EVENT_LONG_PRESS);
            }
        }
    }
    ...
    protected void doInitialization() {
        ...
        if (thing.getChannels().isEmpty()) {
            ...
            relayConfiguration.put(CONFIGURATION_INVERT, Boolean.FALSE);
            ...
            digitalInputConfiguration.put(CONFIGURATION_INVERT, Boolean.FALSE);
            ...
            PROPERTY_TYPE_MAP.keySet().stream().forEach(property -> {
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    uid = new ChannelUID(thing.getUID(), channelId + "#" + TIMESTAMP_CHANNEL_NAME);
                    ...
                    uid = new ChannelUID(thing.getUID(), channelId + "#" + LAST_STATE_DURATION_CHANNEL_NAME);
                    ...
                    if (PROPERTY_DIGITAL_INPUT.equals(property)) {
                        uid = new ChannelUID(thing.getUID(), channelId + "#" + CHANNEL_TYPE_PUSH_BUTTON_TRIGGER);
    ...
    public void dataReceived(String port, Double value) {
        ...
        // ChannelUID channelUID = new ChannelUID(thing.getUID(), port);

        Channel channel = thing.getChannel(portKind);
        ...
        switch (portKind) {
            ...
            case PROPERTY_ANALOG_INPUT:
                AnalogInputConfiguration config = channel.getConfiguration().as(AnalogInputConfiguration.class);
            ...
            case PROPERTY_DIGITAL_INPUT:
                DigitalInputConfiguration config2 = channel.getConfiguration().as(DigitalInputConfiguration.class);
                if (config2.debouncePeriod != 0
                        && now.isBefore(portData.timestamp.plus(config2.debouncePeriod, ChronoUnit.MILLIS))) {
                    return;
                }
                if (value == 1 || value == 0 && config2.swapValue) {
                    state = OpenClosedType.CLOSED;
                    triggerPushButtonChannel(channel, EVENT_PRESSED);
                    if (config2.longPressTime != 0) {
                        scheduler.schedule(new LongPressEvaluator(channel.getUID(), port, portData, config2.swapValue),
        ...
        updateState(channel.getUID() + "#" + TIMESTAMP_CHANNEL_NAME, new DateTimeType(now));
        updateState(channel.getUID() + "#" + LAST_STATE_DURATION_CHANNEL_NAME,
    ...
    protected void triggerPushButtonChannel(Channel channel, String event) {
        ...
        triggerChannel(channel.getUID() + "#" + CHANNEL_TYPE_PUSH_BUTTON_TRIGGER, event);
    }
    ...
    public void handleCommand(ChannelUID channelUID, Command command) {
        ...
        if (channelId.startsWith(PROPERTY_RELAY_OUTPUT) && command instanceof OnOffType) {
            ...
            int targetValue = (OnOffType) command == OnOffType.ON ? config.swapValue ? 0 : 1 : config.swapValue ? 1 : 0;
